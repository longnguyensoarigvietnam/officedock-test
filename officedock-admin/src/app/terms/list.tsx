...
const ListTerms = () => {
  ...
  const [dataTerms, setDataTerms] = useState<Term[]>([]);
  ...
  // Set ID term for delete
  const [idTermChoose, setIdTermChoose] = useState<number>();
  ...
  // Delete term
  const handleOpenDeleteTermModal = (id: number) => {
  ...
  const handleConfirmDeleteTerm = () => {
    if (idTermChoose) {
      setIsLoading(true);
      deleteTerm(idTermChoose);
      return;
    }
  };

  const postDeleteTerm = async (id: number) => {
  ...
  const { mutate: deleteTerm } = useMutation(postDeleteTerm, {
    onSuccess: async () => {
      showToast({
  ...
  return (
    <Fragment>
      ...
      <div className="flex justify-end">
        <Link href={pageRouters.TERM_CREATE.href}>
      ...
      <div className="w-full">
        <Table className="bg-white !rounded-lg">
          ...
          <TableBody>
            {dataTerms && dataTerms.length ? (
              dataTerms.map((element, index) => (
                <tr key={index}>
                  ...
                  <td className="w-52">
                    <div className="flex w-full gap-2 justify-center">
                      <Link
                        href={pageRouters.TERM_DETAIL.href(`${element.id}`)}>
                        <ImageRound
                          name="Detail"
                      ...
                      <>
                        <Link
                          href={pageRouters.TERM_EDIT.href(`${element.id}`)}>
                          <ImageRound
                            name="Edit"
                            src={'/icons/edit.svg'}
                            className={`w-6 h-6 hover:cursor-pointer`}
                        ...
                        {element.isConfirmed == false ? (
                          <>
                            <ImageRound
                              ...
                              className={`w-6 h-6 hover:cursor-pointer`}
                              onClick={() =>
                                handleOpenDeleteTermModal(element.id)
                              }
                            />
                          </>
                        ) : (
