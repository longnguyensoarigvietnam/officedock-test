...
interface UseListShopItemProps {
  screenName?: string;
...
const useListShopItem = ({
  type,
  onSuccess,
  onError,
  onSettled,
  screenName,
}: UseListShopItemProps) => {
  const { data: session } = useSessionCache();
  const token = session?.accessToken;

  /**
   * getListShopItem: load the first page if pageParam has no URL
   * Otherwise if pageParam is a URL (string), call that URL directly
   */
  const getListShopItem = async ({
    pageParam,
    signal,
  }: {
    ...
    if (typeof pageParam === 'string') {
      ...
      apiUrl = `${apiRouters.SHOP_ITEMS}?${params.toString()}`;
    }
  ...
  const {
    ...
    queryKey: ['getListShopItem', screenName, type],
    queryFn: ({ pageParam, signal }) => getListShopItem({ pageParam, signal }),
    enabled: !!token,
    retry: 0,
    refetchOnMount: true,
  ...
  return {
    shopItemList: data?.pages?.flatMap((p) => p?.results ?? []) ?? [],
    fetchNextPage,
    hasNextPage,
    isFetchingNextPage,
    isFetched,
...
export default useListShopItem;
