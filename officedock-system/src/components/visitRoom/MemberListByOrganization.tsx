...
export const MemberListByOrganization = ({
  ...
  return (
    <div
      ...
      className="w-[750px] h-[90%] overflow-y-auto customized-scrollbar p-5 absolute top-1/2 -translate-y-1/2 right-[30px] font-medium text-white border border-white rounded-3xl flex flex-col gap-5">
                ...
                organization.orgInfo.users.map((user) => {
                  return (
                    <div
                      ...
                      className="w-[157px] bg-white rounded-[14px] h-[50px] px-[20px] py-[10px] flex items-center gap-2 hover:cursor-pointer"
                      onClick={() =>
                        router.push(pageRouters.VISIT_ROOM_DETAIL.href(String(user.id)))
                      }>
                      <CustomUserAvatar
