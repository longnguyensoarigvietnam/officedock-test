          ...
          return (
            <>
              <div className={`relative h-full ${label ? 'mt-1' : ''}`}>
                <ListboxButton
                  style={styleClass}
                  className={`relative  w-full cursor-pointer leading-5.5 !text-center rounded bg-white py-2.5 text-gray-900 shadow-sm border text-base focus-visible:!outline-none focus-visible:!shadow-none ${disabled && 'opacity-55'} ${errorClasses}  ${openByDefault ? 'flex justify-center items-center pr-0 pl-0' : 'pl-3.5 pr-10'} ${className}`}>
                  <span
                    ...
                    {selected ? (
                      <>
                        ...
                        <span
                          className={`${selected.imgComponent && 'ml-2'} block truncate ${labelClass} ${classActive} `}>
                          {selected.label}
                        </span>
                      </>
                    ) : (
                ...
                <Transition
                  ...
                  leaveTo="opacity-0">
                  <ListboxOptions
                    className={`absolute z-20 mt-1 max-h-56 border-[1px] border-[#77858F] w-full overflow-auto rounded bg-white text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none ${classNameOption}`}>
                    {searchOption && (
