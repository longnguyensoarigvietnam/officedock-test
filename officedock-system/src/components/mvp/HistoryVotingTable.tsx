...
export const HistoryVotingTable = ({
  ...
  return (
    <div
      ...
      className="w-[700px] h-[90%] p-[30px] absolute top-1/2 -translate-y-1/2 right-[30px] font-medium text-white border border-white rounded-3xl">
      {/* Header */}
      ...
      <div
        ...
        className={`overflow-y-auto overflow-x-hidden customized-scrollbar h-fit rounded-[14px] max-h-[calc(100%_-_30px)] w-full mt-3 flex flex-col gap-[2px] ${!isLoadingList && !historyList.length ? 'bg-white h-full' : ''}`}>
        {isLoadingList ? (
          <div>
            <RowSkeleton
              numberOfRows={6}
