...
const CalendarSkeleton: React.FC<CalendarSkeletonProps> = ({
  ...
  return (
    <div className={`flex flex-col h-[1516px] mt-[30px] pl-[25px] w-full bg-[#E6F3FB] rounded-md overflow-hidden ${className}`}>
      <div className="flex-1 grid grid-rows-[repeat(auto-fill,_87px)]">
        {Array.from({
          length: 26,
        }).map((_, hourIndex) => (
          <div key={hourIndex} className="flex ml-[-10px]">
            <div className="w-16 h-[87px] relative top-[0px] bg-[#E6F3FB] flex justify-center ">
