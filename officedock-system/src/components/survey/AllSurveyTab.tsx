...
type AllSurveyTabProp = {
  handleAnswer: (id: number, isMySurvey?: boolean) => void;
  handleDelete: (id: number) => void;
};

const AllSurveyTab = ({ handleAnswer, handleDelete }: AllSurveyTabProp) => {
  const {
    surveyList,
    fetchNextPage,
    hasNextPage,
                ...
                return (
                  <>
                    ...
                    {(index !== surveyList.length - 1 ||
                      surveyList.length < 7) && (
                      <div className=" w-full py-[14px] bg-white">
                        <div className="bg-[#409EDE] h-[2px]"></div>
                      </div>
                    )}
                  </>
                );
