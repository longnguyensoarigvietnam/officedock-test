...
function StatisticTagCalendar() {
  ...
  const handleSelectTimeOption = (option: TimeOptionsType) => {
    ...
    if (isDataCheckCompare) {
      ...
      if (compareMode === TimeCompareOptionsType.PREVIOUS_YEAR) {
        const { start: compareStart, end: compareEnd } =
          ...
          : option === TimeOptionsType.HALF_YEAR
            ...
            : {
                start: handleSetStartDateBefore(option, newStartDate) as Date,
                  ...
                  option === TimeOptionsType.YESTERDAY
                    ? (handleSetStartDateBefore(option, newStartDate) as Date)
                    : (handleSetStartDateBefore(option, newEndDate) as Date),
              };
      setDataStartDateCompare(compareStart);
      setDataEndDateCompare(compareEnd);
    }
  ...
  const handlePrevCalendar = () => {
    if (isTypeTime === TimeOptionsType.YEAR) {
      ...
      if (isDataCheckCompare && dataStartDateCompare && dataEndDateCompare) {
        ...
        setDataStartDateCompare(new Date(compareYear, 0, 1));
        setDataEndDateCompare(new Date(compareYear, 11, 31));
      }
      return;
    }

    if (isTypeTime === TimeOptionsType.MONTH) {
      const y = dataStartDate.getFullYear();
      const m = dataStartDate.getMonth();
      const prevMonthIndexRaw = m - 1;
      const prevMonthYear = y + Math.floor(prevMonthIndexRaw / 12);
      ...
      if (isDataCheckCompare && dataStartDateCompare && dataEndDateCompare) {
        const cy = dataStartDateCompare.getFullYear();
        const cm = dataStartDateCompare.getMonth();
        const cPrevMonthIndexRaw = cm - 1;
        const cPrevMonthYear = cy + Math.floor(cPrevMonthIndexRaw / 12);
        const cPrevMonthIndex = ((cPrevMonthIndexRaw % 12) + 12) % 12;
        setDataStartDateCompare(new Date(cPrevMonthYear, cPrevMonthIndex, 1));
        setDataEndDateCompare(new Date(cPrevMonthYear, cPrevMonthIndex + 1, 0));
      }
      return;
    }
  ...
  const handleNextCalendar = () => {
    ...
    if (isTypeTime === TimeOptionsType.MONTH) {
      const y = dataStartDate.getFullYear();
      const m = dataStartDate.getMonth();
      const nextMonthIndexRaw = m + 1;
      const nextMonthYear = y + Math.floor(nextMonthIndexRaw / 12);
      ...
      if (isDataCheckCompare && dataStartDateCompare && dataEndDateCompare) {
        const cy = dataStartDateCompare.getFullYear();
        const cm = dataStartDateCompare.getMonth();
        const cNextMonthIndexRaw = cm + 1;
        const cNextMonthYear = cy + Math.floor(cNextMonthIndexRaw / 12);
        const cNextMonthIndex = ((cNextMonthIndexRaw % 12) + 12) % 12;
        setDataStartDateCompare(new Date(cNextMonthYear, cNextMonthIndex, 1));
        setDataEndDateCompare(new Date(cNextMonthYear, cNextMonthIndex + 1, 0));
      }
      return;
    }
            ...
            <div className="flex-1">
              ...
              <div>
                <div
                  ...
                  <span>開始日</span>
                  <div
                    className={`w-[135px] h-[34px] flex items-center cursor-pointer justify-center rounded-md border ${isStartButtonClicked ? 'border-primary' : 'border-[#77858F]'} ${isErrorData.start && '!border-red-500'}`}>
                ...
                <div
                  ...
                  <span>終了日</span>
                  <div
                    className={`w-[135px] h-[34px] flex items-center cursor-pointer justify-center rounded-md border ${isEndButtonClicked ? 'border-primary' : 'border-[#77858F]'} ${isErrorData.end && '!border-red-500'}`}>
...
export default StatisticTagCalendar;
