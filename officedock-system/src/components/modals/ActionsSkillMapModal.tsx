...
export type ActionsSkillMapModalRef = {
...
const ActionsSkillMapModal = forwardRef<
  ...
  (
    ...
    const defaultValues = useMemo<SkillMapFormData>(() => {
      ...
      if (skillMapEditDetail) {
        const getStepDetail = (step: string): StepFormDataDetail | null => {
          const detail = skillMapEditDetail.find(
    ...
    useImperativeHandle(ref, () => ({
      setServerErrors: (serverError) => {
        handleServerFormErrors(serverError, setError, control, true);
      },
    ...
    const LevelUpConditions = ({
      ...
      return (
        <div>
          <p className="text-primary text-md font-medium leading-none">
            {levelTitle}
          </p>
          <p className="text-sm font-medium mt-5 leading-none">
            レベルアップ条件
          </p>
          ...
          <div>
            <p className="text-sm font-medium mb-[14px] mt-5 leading-none">
              振り返り項目
            </p>
