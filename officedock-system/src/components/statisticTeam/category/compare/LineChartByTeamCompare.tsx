...
const LineChartByTeamCompare = ({
  ...
  useEffect(() => {
    if (!standardTableData?.length || !compareTableData?.length) return;
    setTableData(mergeCategories([...standardTableData, ...compareTableData]));
          ...
          hideTimeout = setTimeout(() => {
            if (tooltipRef.current) {
              ...
              chart?.data.datasets.forEach((ds: any, index: number) => {
                ...
                meta.dataset.options.borderWidth = 3; // or original width
                meta.dataset.options.borderColor =
                  ds.originalColor || ds.borderColor;
              });
              chart?.update('none');
            }
            hideTimeout = null;
  ...
  useEffect(() => {
    if (
      !statisticUserTaskDurationsList?.length ||
      !statisticUserTaskDurationsCompareList?.length
    ) {
      return;
    }
    ...
    const loadImages = async () => {
      ...
      const standardImagePromises = statisticUserTaskDurationsList.map((item) =>
  ...
  useEffect(() => {
    ...
    if (statisticUserTaskDurationsList.length > 0) {
      statisticUserTaskDurationsList.forEach((userTaskDuration) => {
        ...
        const compareCategory = statisticUserTaskDurationsCompareList.find(
    ...
    if (statisticUserTaskDurationsCompareList.length > 0) {
      statisticUserTaskDurationsCompareList.forEach((userTaskDuration) => {
        comparedLabels.push({
          color: userTaskDuration.user.avatarColor || getRandomColor(),
          name: userTaskDuration.user.fullName,
        });
        const standardCategory = statisticUserTaskDurationsList.find(
          (c) => c.user.id == userTaskDuration.user.id,
        );
        datasets.push({
          label: userTaskDuration.user.fullName,
        ...
        return (
          <div className="flex items-start px-[18px]">
            ...
            <div className="w-full">
              <div
                className={`flex flex-col items-start gap-2 ${collapseStatus && 'mb-8'}`}>
              ...
              {collapseStatus &&
                ...
                info.row.original?.userList.length > 0 && (
                  <div className="flex flex-col gap-8">
                    {info.row.original?.userList.map((user) => {
                      return (
                        <div
      ...
      cell: (info) => {
        ...
        return (
          <div className="w-full px-[18px]">
            <div className={`flex flex-col gap-2 ${collapseStatus && 'mb-8'}`}>
